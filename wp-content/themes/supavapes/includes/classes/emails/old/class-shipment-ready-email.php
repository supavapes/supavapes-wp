 ...
 * @extends \WC_Email
 */
class Shipment_Ready_Email extends WC_Email {
	/**
	 * Set email defaults.
	 *
	 * @since 1.0.0
	...
	public function __construct() {
		...
		$this->heading        = __( 'Ready For Shipment', 'hello-elementor-child' );

		// translators: placeholder is {blogname}, a variable that will be substituted when email is sent out.
		$this->subject = sprintf( _x( '[%s] Ready For Shipment', 'default email subject for rental ggreement being sent to the customer', 'hello-elementor-child' ), '{blogname}' );

		// Template paths.
		$this->template_html  = 'shipment-ready-email-html.php';
		$this->template_plain = 'plain/shipment-ready-email-plain.php';

		add_action( 'hello_elementor_shipment_ready_email', array( $this, 'hello_elementor_hello_elementor_shipment_ready_callback' ), 20, 2 );
	 ...
	 * @since 1.0.0
	 */
	public function hello_elementor_hello_elementor_shipment_ready_callback( $first_name, $last_name ) {
	...
	public static function create_object( $first_name, $last_name ) {
		 ...
		 * @since 1.0.0
		 */
		return apply_filters( 'send_shipment_ready_email_object', $item_object );
	}
