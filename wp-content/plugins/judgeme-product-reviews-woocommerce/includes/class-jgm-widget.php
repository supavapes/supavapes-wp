...
class JGM_Widget {
	...
	public function init() {
		...
		add_shortcode( 'jgm-popup-widget', array( $this, 'judgeme_popup_widget' ) );
	...
	public function judgeme_popup_widget() {
		$widget = get_option( 'judgeme_widget_popup_widget' );

		if ( empty( $widget ) ) {
			$widget = json_decode( wp_remote_retrieve_body( wp_safe_remote_get( "{$this->api_host}/widgets/popup_widget?api_token={$this->token}&shop_domain={$this->domain}" ) ), true );
			update_option( 'judgeme_widget_popup_widget', $widget, true );
		}

		ob_start();
		include JGM_PLUGIN_PATH . 'templates/judgeme/popup-widget-template.php';
		$output = ob_get_clean();
