...
function final_pos_setup_wizard() {
    ...
    ?>
        <header>
            <img src="<?php echo esc_url(plugins_url('finalpos/assets/img/ui/final-logo.png')); ?>" alt="<?php esc_attr_e('Final Logo', 'final-pos'); ?>" class="final-logo">
        </header>
        ...
        <div class="wizard-content">
            ...
            <div class="notice notice-error">
                <p><?php esc_html_e('You need to approve the WooCommerce Auth Screen to use FinalPOS sync features', 'final-pos'); ?></p>
            </div>
            <?php endif; ?>
            ...
            <div class="wizard-step active" id="step1">
                <div class="content">
                    <h1><?php esc_html_e('Welcome to Final POS Builder! Here are the steps to set up your website with Final:', 'final-pos'); ?></h1>
                    ...
                    <div class="login-options">
                        ...
                        <button type="button" class="signin-btn" onclick="nextStep(2)"><?php esc_html_e('Sign In to an existing account', 'final-pos'); ?></button>
                        <p class="signup-text"><?php esc_html_e('Don\'t have an account yet? ', 'final-pos'); ?><a href="#" id="signUpLink"><?php esc_html_e('Sign Up', 'final-pos'); ?></a></p>
                    </div>
                    <div class="overview">
                        ...
                        <div class="icon-boxes">
                            <div class="icon-box-item">
                                ...
                                <p><?php esc_html_e('Final is the world\'s first point of sale builder. Make a unique and customized POS experience matching your needs.', 'final-pos'); ?></p>
                            </div>
                            <div class="icon-box-item">
                                <span class="material-symbols-outlined">sync</span>
                                <h3><?php esc_html_e('Always in sync with your online store', 'final-pos'); ?></h3>
                                <p><?php esc_html_e('Seamlessly integrate your brick-and-mortar store with your online presence, ensuring real-time synchronization of inventory and data.', 'final-pos'); ?></p>
                            </div>
                            <div class="icon-box-item">
                                <span class="material-symbols-outlined">devices</span>
                                <h3><?php esc_html_e('Desktop, mobile, kiosk and everything else.', 'final-pos'); ?></h3>
                                <p><?php esc_html_e('Design your POS interface for any device, from desktop to mobile, and bring your custom build to life with the Final POS app.', 'final-pos'); ?></p>
                            </div>
                            <div class="icon-box-item">
                                <span class="material-symbols-outlined">settings</span>
                                <h3><?php esc_html_e('Customizable and flexible', 'final-pos'); ?></h3>
                                <p><?php esc_html_e('Enjoy extreme control over your checkout process with dynamic fields, automation, and conditional logic.', 'final-pos'); ?></p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="wizard-step" id="step3">
                <div class="two-column">
                    <div class="column-left">
                        <div class="content">
                            <button type="button" class="back-btn" onclick="prevStep(2)">← <?php esc_html_e('Back', 'final-pos'); ?></button>
                                ...
                                <?php foreach (['products' => 'Products', 'orders' => 'Orders', 'customers' => 'Customers'] as $key => $label): ?>
                                    <div class="toggle-item">
                                        <div class="toggle-label">
                                            ...
                                            <div>
                                                <label for="<?php echo esc_attr($key); ?>" class="toggle-labeltext"><?php echo esc_html($label); ?></label>
                                                <p class="toggle-description"><?php esc_html_e('Syncing all categories.', 'final-pos'); ?> <?php echo esc_html(ucfirst($label)); ?></p>
                                                <?php if ($key !== 'customers'): ?>
                                                    <a href="#" class="configure-link" data-popup="<?php echo esc_attr($key); ?>"><?php esc_html_e('Configure your sync terms', 'final-pos'); ?></a>
                                                <?php endif; ?>
                                            </div>
                                        </div>
                            ...
                            <button type="button" class="next-btn" onclick="nextStep(4)"><?php esc_html_e('Continue', 'final-pos'); ?></button>
                        </div>
                    </div>
                    <div class="column-right" style="background-image: url('<?php echo esc_url(plugins_url('finalpos/assets/img/wizard-step1.jpg')); ?>');"></div>
                </div>
            </div>
                ...
                <div class="category-list-container">
                    ...
                    <div class="category-list">
                        ...
                        function display_category_hierarchical($category, $depth = 0) {
                            ...
                            echo wp_kses_post($indent) . '<input type="checkbox" name="product_category[]" value="' . esc_attr($category->term_id) . '" ' . esc_attr($checked) . '>';
...
// Redirect nur bei Plugin-Aktivierung
function final_pos_activation_redirect($plugin) {
    if($plugin == plugin_basename(FINAL_POS_PLUGIN_FILE)) {
        exit(wp_redirect(admin_url('admin.php?page=final-pos-setup')));
    }
}
add_action('activated_plugin', 'final_pos_activation_redirect');
