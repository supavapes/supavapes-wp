 ...
 * @extends \WC_Email
 */
class Payment_Multiple_Fail_Multiple_Attempt_Email extends WC_Email {
	/**
	 * Set email defaults.
	 *
	 * @since 1.0.0
	...
	public function __construct() {
		...
		$this->title       = __( 'Payment Fail Attempt', 'supavapes' );
		$this->description = __( 'An email sent to the customer when multiple payment fail attempt order.', 'supavapes' );

		// For admin area to let the user know we are sending this email to the customer.
		$this->customer_email = true;
		$this->heading        = __( 'Payment Fail Attempt', 'supavapes' );

		// translators: placeholder is {blogname}, a variable that will be substituted when email is sent out.
		$this->subject = sprintf( _x( '[%s] Payment Fail Attempt', 'default email subject for rental ggreement being sent to the customer', 'supavapes' ), '{blogname}' );

		// Template paths.
		$this->template_html  = 'multiple-payment-fail-attempt-email-html.php';
		$this->template_plain = 'plain/multiple-payment-fail-attempt-email-plain.php';

		add_action( 'hello_elementor_multiple_payment_fail_attempt_ready_email', array( $this, 'hello_elementor_multiple_payment_fail_attempt_ready_callback' ), 20, 2 );
	 ...
	 * @since 1.0.0
	 */
	public function hello_elementor_multiple_payment_fail_attempt_ready_callback( $first_name, $last_name ) {
