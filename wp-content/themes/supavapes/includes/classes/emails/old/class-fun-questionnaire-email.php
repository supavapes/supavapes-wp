 ...
 * @extends \WC_Email
 */
class Fun_Questionnaire_Email extends WC_Email {
	/**
	 * Set email defaults.
	 *
	 * @since 1.0.0
	...
	public function __construct() {
		...
		$this->description = __( 'An email sent to the customer pass the quize.', 'hello-elementor-child' );

		// For admin area to let the user know we are sending this email to the customer.
		$this->customer_email = true;
		$this->heading        = __( 'Won the quize', 'hello-elementor-child' );

		// translators: placeholder is {blogname}, a variable that will be substituted when email is sent out.
		$this->subject = sprintf( _x( '[%s] Congratulations', 'default email subject for rental ggreement being sent to the customer', 'hello-elementor-child' ), '{blogname}' );

		// Template paths.
		$this->template_html  = 'fun-questionnarie-email-html.php';
		$this->template_plain = 'plain/fun-questionnarie-email-plain.php';

		add_action( 'hello_elementor_fun_questionnaire_email', array( $this, 'hello_elementor_hello_elementor_fun_questionnaire_callback' ), 20, 2 );
	 ...
	 * @since 1.0.0
	 */
	public function hello_elementor_hello_elementor_fun_questionnaire_callback( $first_name, $last_name ) {
	...
	public static function create_object( $first_name, $last_name ) {
		 ...
		 * @since 1.0.0
		 */
		return apply_filters( 'send_fun_questionnaire_email_object', $item_object );
	}
