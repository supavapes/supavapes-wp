...
// Check the UI choice
$ui_choice = "classic"; // enable only classic UI for now
...
if ($ui_choice !== 'classic') {
    ...
    {
        update_option('default_admin_color', 'classic');
    }
...
{
    $wizard_status = get_option('final_wizard_status', '');
    if ($plugin === plugin_basename(__FILE__) && $wizard_status !== 'completed_sync') {
        wp_safe_redirect(admin_url('admin.php?page=final-pos-setup'));
        exit;
    }
}
...
register_uninstall_hook(__FILE__, 'final_pos_uninstall');
function final_pos_uninstall()
{

    global $wpdb;
    $wpdb->query($wpdb->prepare('DELETE FROM `' . $wpdb->prefix . 'woocommerce_api_keys` WHERE `description` LIKE "%s"', '%FinalPOS - API%'));
    $wpdb->query($wpdb->prepare('DELETE FROM `' . $wpdb->prefix . 'wc_webhooks` WHERE `delivery_url` LIKE "%s"', '%finalpos.com%'));


    $options = [
        'final_wizard_status',
        'default_admin_color',
        'final_pos_access_token',
        'final_pos_company_id',
        'final_pos_consumer_key',
        'final_pos_consumer_secret',
        'final_pos_token',
        'final_pos_error',
        'final_pos_logo_url',
        'final_sync_error',
        'bulk_editor_custom_columns'
    ];
