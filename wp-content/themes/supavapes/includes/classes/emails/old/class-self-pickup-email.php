 ...
 * @extends \WC_Email
 */
class Self_Pickup_Email extends WC_Email {
	/**
	 * Set email defaults.
	 *
	 * @since 1.0.0
	...
	public function __construct() {
		...
		$this->title       = __( 'Self Pickup', 'hello-elementor-child' );
		...
		$this->heading        = __( 'Self Pickup', 'hello-elementor-child' );

		// translators: placeholder is {blogname}, a variable that will be substituted when email is sent out.
		$this->subject = sprintf( _x( '[%s] Self Pickup', 'default email subject for rental ggreement being sent to the customer', 'hello-elementor-child' ), '{blogname}' );

		// Template paths.
		$this->template_html  = 'self-pickup-email-html.php';
		$this->template_plain = 'plain/self-pickup-email-plain.php';

		add_action( 'hello_elementor_self_pickup_email', array( $this, 'hello_elementor_hello_elementor_self_pickup_callback' ), 20, 2 );
	 ...
	 * @since 1.0.0
	 */
	public function hello_elementor_hello_elementor_self_pickup_callback( $first_name, $last_name ) {
	...
	public static function create_object( $first_name, $last_name ) {
		 ...
		 * @since 1.0.0
		 */
		return apply_filters( 'send_self_pickup_object', $item_object );
	}
