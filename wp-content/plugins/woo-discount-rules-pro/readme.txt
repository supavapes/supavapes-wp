...
= 2.6.0 - 08/05/23 =
* Add: Support WooCommerce High-Performance order storage feature.
* Improvement: Query optimization while load coupon in backend.
* Improvement: Licence URL update.
* Fix: Loading range on backend - disappear when remove the first one.

= 2.5.3 - 16/01/23 =
* Fix: Validation fix on creating rules.
* Fix: The cheapest product does not receive a discount when variant together option is enabled.
* Fix: Error on variation is not an array.
* Fix: Condition doesn't match when have Match any option with user role condition.
* Fix: On-sale page shows incorrect data while have attribute in filters.
...
* Improvement - Translation for Free shipping.
