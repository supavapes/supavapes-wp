	...
	class BVCallbackRequest {
		...
		public function __construct($account, $in_params, $settings) {
			...
			$this->pubkey_name = !empty($in_params['pubkeyname']) ?
					CWSAccount::sanitizeKey($in_params['pubkeyname']) : 'm_public';
			$this->bvprmsmac = !empty($in_params['bvprmsmac']) ? CWSAccount::sanitizeKey($in_params['bvprmsmac']) : "";
		...
		public function authFailedResp() {
			$api_public_key = CWSAccount::getApiPublicKey($this->settings);
			$default_secret = CWSRecover::getDefaultSecret($this->settings);
			$default_account_pubkey = CWSAccount::getDefaultPublicKey();
			$bvinfo = new CWSInfo($this->settings);
			$resp = array(
				"request_info" => $this->info(),
				"bvinfo" => $bvinfo->info(),
