...
function update_sv_product_slider() {

	// Get category slug and limit from AJAX request
    $category_slug = isset($_POST['category']) ? sanitize_text_field($_POST['category']) : 'monthly-deals';
    $limit = isset($_POST['limit']) ? intval($_POST['limit']) : 8;

	// WP_Query to fetch products from the specific category with the limit
    $args = array(
        'post_type' => 'product',
		...
		while ($products->have_posts()) {
			...
			<div class="sv-our-product-box">
				...
				if ( $product_data && method_exists( $product_data, 'get_type' ) ) {
					$product_type = $product_data->get_type();
					if( $product_type === 'variable' ){?>
						<a href="<?php echo esc_url(get_permalink()); ?>" class="sv-shop-btn"><?php echo esc_html__('Select Options','supavapes'); ?></a>
					<?php }else{?>
						<a href="<?php echo esc_url(get_permalink()); ?>" class="sv-shop-btn"><?php echo esc_html__('Shop Now','supavapes'); ?></a>
					<?php }?>
				<?php }?>
...
if ( ! function_exists( 'supavapes_woocommerce_cart_calculate_fees_callback' ) ) {
	...
	function supavapes_woocommerce_cart_calculate_fees_callback( $cart ) {
		...
		$total_vaping_liquid = 0;

		// Loop through cart items to sum up the vaping liquid volume.
		foreach ( $cart->get_cart() as $cart_item ) {
			$product = $cart_item['data'];

			// Get _vaping_liquid meta value for simple and variation products.
			$vaping_liquid = get_post_meta( $product->get_id(), '_vaping_liquid', true );
			...
			if ( is_numeric( $vaping_liquid ) && $vaping_liquid > 0 ) {
				// Multiply the vaping liquid volume by the cart item quantity.
				$total_vaping_liquid += $vaping_liquid * $cart_item['quantity'];
			}
		}
		echo "Total vaping liquid: ".$total_vaping_liquid;
		// If the total vaping liquid is greater than 0, calculate and apply taxes.
		if ( $total_vaping_liquid > 0 ) {
			// Calculate Ontario and Federal taxes based on the total vaping liquid.
