...
<!-- WISHLIST TABLE -->
<table
	class="shop_table test-class cart wishlist_table wishlist_view traditional responsive <?php echo $no_interactions ? 'no-interactions' : ''; ?> <?php echo $enable_drag_n_drop ? 'sortable' : ''; ?> "
		...
		<?php if ( $show_stock_status ) : ?>
			...
			<th class="product-stock-status">
				<span class="nobr">
					 ...
					 * @return string
					 */
					echo esc_html( apply_filters( 'yith_wcwl_wishlist_view_stock_heading', __( 'Stock', 'yith-woocommerce-wishlist' ), $wishlist ) );
					?>
				</span>
			</th>
		<?php endif; ?>
				...
				<tr id="yith-wcwl-row-<?php echo esc_attr( $item->get_product_id() ); ?>" data-row-id="<?php echo esc_attr( $item->get_product_id() ); ?>">
					...
					<?php if ( $show_stock_status ) : ?>
						<td class="product-stock-status">
							 ...
							 * @return string
							 */
							$stock_status_html = 'out-of-stock' === $item->get_stock_status() ? '<span class="wishlist-out-of-stock">' . esc_html( apply_filters( 'yith_wcwl_out_of_stock_label', __( 'Out of stock', 'yith-woocommerce-wishlist' ) ) ) . '<img src="/wp-content/uploads/2024/05/sad-icon.png" /></span>' : '<span class="wishlist-in-stock">' . esc_html( apply_filters( 'yith_wcwl_in_stock_label', __( 'In Stock', 'yith-woocommerce-wishlist' ) ) ) . '<img src="/wp-content/uploads/2024/05/happy-icon.png" /></span>';
					...
					<?php if ( $show_last_column ) : ?>
						<td class="product-add-to-cart">
							...
							if ( $repeat_remove_button ) :
								 ...
								 */
								?>
								<a href="<?php echo esc_url( $item->get_remove_url() ); ?>" class="remove_from_wishlist button" title="<?php echo esc_html( apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'yith-woocommerce-wishlist' ) ) ); ?>"><i class="fa fa-trash"></i></a>
							<?php endif; ?>
							<?php
							/**
					...
					<?php if ( $show_remove_product ) : ?>
						<td class="product-remove">
							<div>
								 ...
								 */
								?>
								<a href="<?php echo esc_url( $item->get_remove_url() ); ?>" class="remove remove_from_wishlist" title="<?php echo esc_html( apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'yith-woocommerce-wishlist' ) ) ); ?>"><i class="fa fa-trash"></i></a>
							</div>
						</td>
					<?php endif; ?>
					<?php if ( $enable_drag_n_drop ) : ?>
